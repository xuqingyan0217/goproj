...
func (ft *FindProductsTool) InvokableRun(ctx context.Context, argumentsInJSON string, opts ...tool.Option) (string, error) {
	log.Printf("FindProductsTool ctx address: %p", ctx)
...
func (ft *FindProductsTool) Helper(proList []ProductInfo) {
	for _, info := range proList {
		log.Printf("Product ID: %d, Quantity: %d", info.ProductId, info.Quantity)
	}
}
func main() {
