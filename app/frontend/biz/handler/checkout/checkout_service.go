...
// CheckoutRepay .
// @router /checkout/repay [POST]
func CheckoutRepay(ctx context.Context, c *app.RequestContext) {
	var err error
	var req checkout.CheckoutRepayReq
	err = c.BindAndValidate(&req)
	if err != nil {
		utils.SendErrResponse(ctx, c, consts.StatusOK, err)
		return
	...
	resp, err := service.NewCheckoutRepayService(ctx, c).Run(&req)
